...
def analyze_cross_indicators(
    ...
    """
    3차-J FCS + RSI/RMI 종합분석 (Cross Analysis)
    ...
    is_all_pos = (fcs_dir == "UP" and rsi_dir == "UP" and rmi_dir == "UP")
    is_all_neg = (fcs_dir == "DOWN" and rsi_dir == "DOWN" and rmi_dir == "DOWN")
    ...
    if is_all_pos:
        ...
        status_color = "#22c55e"
        reasons.append("FCS 수급, 기술적 지표(RSI/RMI)가 모두 긍정적 방향 (추세 지속 가능성 우수)")
    elif is_all_neg:
        status_label = "🔴 기술·수급 동시 약화"
        status_color = "#ef4444"
        reasons.append("FCS 수급, 기술적 지표가 동반 하향 약화 (위험 관리 및 관망 필요)")
    elif is_conflict:
